...
namespace CoatingMgr
{
    public partial class FormIn : Form
    {
        ...
        private void TbBarCode_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                sqlLiteHelper.InsertValues(_tableName, new string[] { tbBarCode.Text, tbName.Text, " ", tbType.Text, tbWeigth.Text, tbModel.Text, "2019.01.20", tbDate.Text, cbSearchStock.Text, _userName, DateTime.Now.ToString(), "入库", " ", " " });
                UpdateData();
            }
            else
            {
        ...
        private void BtnOk_Click(object sender, EventArgs e)
        {
            if (dgvInStockData.RowCount > 0)
            {
                sqlLiteHelper.InsertDataWithoutIdFromOtherTable(Common.STOCKMANAGERTABLENAME, _tableName);
